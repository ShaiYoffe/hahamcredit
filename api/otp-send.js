...
const ALLOWED_ORIGINS = [
  'https://hahamcredit.com',
  'https://www.hahamcredit.com',
];
...
async function sendInforuSMS(phone, code, ymsg) {
  ...
  const sender = (process.env.OTP_SENDER || 'hahamcredit').trim();
  const message = ymsg;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const ymsg =
      ...
      flow === 'telecom'            ? `קוד אימות: ${code}. לאחר האישור — שיחה נפרדת מנציג תקשורת מורשה להצעת הוזלה על חבילת הטלוויזיה (בנוסף לבקשתך המקורית).` :
